import 'package:billma_assistant/components/TTSEngine.dart';
import 'package:flutter/material.dart';
import 'package:flutter_dialogflow/dialogflow_v2.dart';
import '../components/SpeechListener.dart';
...
class _ChatScreen extends State<ChatScreen> {
  ...
  void initTts() {
    _ttsEngine = new TTSEngine("es-MX","Paulina");
  }
      ...
      child: new Container(
        ...
        child: new Row(
          children: <Widget>[
            ...
            new Container(
              margin: new EdgeInsets.symmetric(horizontal: 4.0),
              child: new SpeechListenerButton(inputText),
            )
          ],
        ),
      ),
  ...
  void Response(query) async {
    ...
    AuthGoogle authGoogle = await AuthGoogle(fileJson: "assets/BillMa-66b701c7c491.json").build();
    Dialogflow dialogflow = Dialogflow(authGoogle: authGoogle,language: Language.spanish);
    AIResponse response = await dialogflow.detectIntent(query);
    _ttsEngine.play(response.getMessage());
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: new AppBar(
        title: new Text("BillMa Assistant"),
      ),
